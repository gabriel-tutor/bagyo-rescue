---
description: Patterns for creating mutation hooks (create, update, delete) using TanStack Query useMutation
globs: **/hooks/query/**/*mutation*.ts, **/_hooks/query/**/*mutation*.ts
alwaysApply: false
---
...
- Forward callback arguments with `...restArgs` rather than naming them (`data, variables, context`). TanStack Query v5.65+ added a 4th `onMutateResult` parameter, and hard-coding the arity breaks consumer overrides when the package is upgraded
...
export function useCreate[Entity]Mutation(args: UseCreate[Entity]MutationArgs = {}) {
  ...
  return useMutation({
    ...
    onError: (...errorArgs) => {
      if (args?.onError) return args.onError(...errorArgs);
      alertApiError(errorArgs[0]);
    },
  });
}
```
